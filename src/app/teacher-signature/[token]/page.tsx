...
export default function TeacherSignaturePage() {
    ...
    useEffect(() => {
        const fetchTokenData = async () => {
            try {
                // Use the new API endpoint for teacher tokens
                const res = await fetch(`/api/teacher-signature/${token}`)
                if (!res.ok) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const data = await res.json()
            // Although POST returns success:true, we might want to refresh missed sessions if returned
            // For now, simpler to just set success
            if (sessionData?.missedSessions) {
                setMissedSessions(sessionData.missedSessions)
            }
    ...
    const handleSignMissedSession = async (session: MissedSession) => {
        ...
        try {
            // Reuse the same API route effectively since it updates by token
            const res = await fetch(`/api/teacher-signature/${session.token}`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ signatureData })
            })
                    ...
                    <CardContent className="space-y-6">
                        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                            <p className="text-sm text-emerald-900">
                                <strong>Formateur:</strong> {sessionData?.teacher.name}
                            </p>
                            <p className="text-sm text-emerald-900">
